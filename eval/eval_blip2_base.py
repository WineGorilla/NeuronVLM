...
用法：
    CUDA_VISIBLE_DEVICES=0 python eval/eval_blip2_base.py
    python eval/eval_blip2_base.py --benchmarks cvbench mmstar
    python eval/eval_blip2_base.py --benchmarks blink --depth_spatial_only
    python eval/eval_blip2_base.py --max_samples 50
"""
import os
import sys
import re
...
def blip2_generate(model, processor, image, question, max_new_tokens=32):
    """BLIP-2 单次推理。"""
    try:
        ...
        # BLIP-2 使用简单的 prompt 格式，不需要 chat template
...
def eval_cvbench(model, processor, max_samples=None):
    ...
    for item in tqdm(ds, desc="CV-Bench"):
        ...
        response = blip2_generate(model, processor, item["image"], prompt)
...
def eval_realworldqa(model, processor, max_samples=None):
    ...
    for i, item in enumerate(tqdm(ds, desc="RealWorldQA")):
        ...
        response = blip2_generate(model, processor, item["image"], question)
...
def eval_blink(model, processor, subtasks=None, max_samples=None):
    ...
    for item in tqdm(all_items, desc="BLINK"):
        ...
        response = blip2_generate(model, processor, concat_path, prompt_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BLIP-2 Base Evaluation")
    parser.add_argument("--benchmarks", type=str, nargs="+",
                        default=["cvbench", "mmstar", "realworldqa", "blink"],
                        choices=["cvbench", "mmstar", "realworldqa", "blink"])
    parser.add_argument("--max_samples", type=int, default=None)
    parser.add_argument("--save_dir", type=str, default="outputs/blip2_base_results")
