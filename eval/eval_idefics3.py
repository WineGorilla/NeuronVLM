"""
Idefics3-8B-Llama3 基础模型评估脚本。

支持 4 个 benchmark：CV-Bench, BLINK, RealWorldQA, MMStar。
使用 Idefics3 的 AutoModelForVision2Seq + AutoProcessor 接口推理。

用法：
    CUDA_VISIBLE_DEVICES=0 python eval/eval_idefics3.py
    python eval/eval_idefics3.py --benchmarks cvbench mmstar
    python eval/eval_idefics3.py --benchmarks blink --depth_spatial_only
    python eval/eval_idefics3.py --max_samples 50

依赖：
    pip install transformers>=4.46.0 torchvision decord pillow
"""
import os
import sys
import re
...
# ══════════════════════════════════════════════════════════════════════════════
# 模型加载（Idefics3 原生 transformers 接口）
# ══════════════════════════════════════════════════════════════════════════════

def load_idefics3():
    from transformers import AutoProcessor, Idefics3ForConditionalGeneration
    print(f"Loading Idefics3: {MODEL_ID}")
...
def idefics3_generate(model, processor, image, question, max_new_tokens=32):
    """Idefics3 推理，使用 apply_chat_template + model.generate()。"""
    try:
...
def eval_cvbench(model, processor, max_samples=None):
    ...
    for item in tqdm(ds, desc="CV-Bench"):
        ...
        response = idefics3_generate(model, processor, item["image"], prompt)
...
def eval_realworldqa(model, processor, max_samples=None):
    ...
    for i, item in enumerate(tqdm(ds, desc="RealWorldQA")):
        ...
        response = idefics3_generate(model, processor, item["image"], question)
...
def eval_blink(model, processor, subtasks=None, max_samples=None):
    ...
    for item in tqdm(all_items, desc="BLINK"):
        ...
        response = idefics3_generate(model, processor, concat_path, prompt_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Idefics3 Base Model Evaluation")
    parser.add_argument("--benchmarks", type=str, nargs="+",
                        default=["cvbench", "mmstar", "realworldqa", "blink"],
                        choices=["cvbench", "mmstar", "realworldqa", "blink"])
    parser.add_argument("--max_samples", type=int, default=None)
    parser.add_argument("--save_dir", type=str, default="outputs/idefics3_results")
    parser.add_argument("--model_id", type=str, default=MODEL_ID,
                        help="Idefics3 model ID (default: HuggingFaceM4/Idefics3-8B-Llama3)")
