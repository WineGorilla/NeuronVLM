"""
InternVL2.5 基础模型评估脚本。

支持 4 个 benchmark：CV-Bench, BLINK, RealWorldQA, MMStar。
使用 InternVL2.5 自带的 model.chat() 接口推理。

用法：
    CUDA_VISIBLE_DEVICES=0 python eval/eval_internvl3_base.py
    python eval/eval_internvl3_base.py --benchmarks cvbench mmstar
    python eval/eval_internvl3_base.py --benchmarks blink --depth_spatial_only
    python eval/eval_internvl3_base.py --max_samples 50
    CUDA_VISIBLE_DEVICES=1 python eval/eval_internvl3_base.py

依赖：
    pip install transformers>=4.37.2 torchvision decord
"""
import os
import sys
import re
...
# ══════════════════════════════════════════════════════════════════════════════
# 模型加载（InternVL3-hf 原生 transformers 接口）
# ══════════════════════════════════════════════════════════════════════════════

def load_internvl25():
    from transformers import AutoProcessor, AutoModelForImageTextToText

    print(f"Loading InternVL: {MODEL_ID}")
...
def internvl_generate(model, processor, image, question, max_new_tokens=32):
    """InternVL3-hf 推理，标准 transformers 接口。"""
    try:
...
def eval_cvbench(model, processor, max_samples=None):
    ...
    for item in tqdm(ds, desc="CV-Bench"):
        ...
        response = internvl_generate(model, processor, item["image"], prompt)
...
def eval_realworldqa(model, processor, max_samples=None):
    ...
    for i, item in enumerate(tqdm(ds, desc="RealWorldQA")):
        ...
        response = internvl_generate(model, processor, item["image"], question)
...
def eval_blink(model, processor, subtasks=None, max_samples=None):
    ...
    for item in tqdm(all_items, desc="BLINK"):
        ...
        response = internvl_generate(model, processor, concat_path, prompt_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="InternVL Base Model Evaluation")
    parser.add_argument("--benchmarks", type=str, nargs="+",
                        default=["cvbench", "mmstar", "realworldqa", "blink"],
                        choices=["cvbench", "mmstar", "realworldqa", "blink"])
    parser.add_argument("--max_samples", type=int, default=None)
    parser.add_argument("--save_dir", type=str, default="outputs/internvl25_base_results")
    parser.add_argument("--model_id", type=str, default=MODEL_ID,
                        help="InternVL2.5 model ID")
    ...
    model, processor = load_internvl25()
    ...
    print(f"\n{'='*60}")
    print(f"  InternVL2.5 Base — Summary")
    print(f"{'='*60}")
