"""
LLaVA-NeXT-LLaMA3-8B 基础模型评估脚本。

支持 4 个 benchmark：CV-Bench, BLINK, RealWorldQA, MMStar。
只测原始模型（无 hook / 无增强），用于获取 baseline 分数。

用法：
    CUDA_VISIBLE_DEVICES=0 python eval/eval_llava_next_base.py
    python eval/eval_llava_next_base.py --benchmarks cvbench mmstar
    python eval/eval_llava_next_base.py --benchmarks blink --depth_spatial_only
    python eval/eval_llava_next_base.py --max_samples 50
"""
import os
import sys
import re
...
def llava_next_generate(model, processor, image, question, max_new_tokens=32):
    """LLaVA-NeXT 单次推理。"""
    try:
...
def eval_cvbench(model, processor, max_samples=None):
    ...
    for item in tqdm(ds, desc="CV-Bench"):
        ...
        response = llava_next_generate(model, processor, item["image"], prompt)
...
def eval_realworldqa(model, processor, max_samples=None):
    ...
    for i, item in enumerate(tqdm(ds, desc="RealWorldQA")):
        ...
        response = llava_next_generate(model, processor, item["image"], question)
...
def eval_blink(model, processor, subtasks=None, max_samples=None):
    ...
    for item in tqdm(all_items, desc="BLINK"):
        ...
        response = llava_next_generate(model, processor, concat_path, prompt_text)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LLaVA-NeXT-LLaMA3-8B Base Evaluation")
    parser.add_argument("--benchmarks", type=str, nargs="+",
                        default=["cvbench", "mmstar", "realworldqa", "blink"],
                        choices=["cvbench", "mmstar", "realworldqa", "blink"])
    parser.add_argument("--max_samples", type=int, default=None)
    parser.add_argument("--save_dir", type=str, default="outputs/llava_next_base_results")
    ...
    print(f"\n{'='*60}")
    print(f"  LLaVA-NeXT-LLaMA3-8B Base — Summary")
    print(f"{'='*60}")
