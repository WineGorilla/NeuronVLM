"""
FastV 评估脚本 — 在 Qwen2.5-VL 上复现 FastV 算法。

FastV 核心思想 (Chen et al., ECCV 2024):
  在 LLM decoder 的第 K 层之后，根据 attention score 对视觉 token 排序，
  剪掉得分最低的 R% 视觉 token（通过 attention mask 屏蔽）。

实现方式：
  通过在 LLM decoder 第 K 层的 forward hook 中，将被剪枝的 visual token
  的 hidden states 替换为其邻近保留 token 的值（scatter 方式），
  而非简单置零（置零会严重干扰模型生成）。

支持 4 个 benchmark：CV-Bench, BLINK, RealWorldQA, MMStar。

用法：
    python eval_new/eval_fastv.py --benchmarks cvbench mmstar
    python eval/eval_fastv.py --fastv_k 2 --fastv_r 0.5
    python eval/eval_fastv.py --fastv_k 3 --fastv_r 0.75 --benchmarks blink --depth_spatial_only
...
# ══════════════════════════════════════════════════════════════════════════════
# FastV: 基于 Attention Score 的 Visual Token 剪枝
# ══════════════════════════════════════════════════════════════════════════════

class FastVWrapper:
    """
    FastV：在第 K 层后，将不重要的 visual token 替换为其最近的保留 token
    （scatter-copy 方式），使其信号被保留 token 淹没，而不破坏序列结构。

    为什么不用置零：
      - 零向量仍被 attention 访问，会产生异常的 attention 分布
      - 导致模型输出乱码
    
    为什么用 scatter-copy：
      - 被剪 token 变成保留 token 的"副本"，attention 访问它们不会出错
      - 等效于让保留 token 的信息被更多引用，被剪 token 不贡献新信息
      - 不需要修改 attention_mask 或模型内部代码
    """

    def __init__(self, model, fastv_k=2, fastv_r=0.5):
        self.model = model
    ...
    def _make_hook(self, layer_idx):
        def hook_fn(module, args, output):
            ...
            if seq_len <= 1 or self._pruned:
                return output

            if layer_idx == self.fastv_k:
                self._do_pruning(hidden_states)

            return output
        return hook_fn

    def _do_pruning(self, hidden_states):
        """
        FastV 剪枝：将不重要 visual token 替换为其最近的保留 token。
        """
        ...
        for b in range(batch_size):
            ...
            n_prune = int(n_img * self.fastv_r)
            if n_prune == 0:
                continue

            n_keep = n_img - n_prune

            # 用 L2 norm 作为重要性度量
            img_hidden = hidden_states[b, img_positions, :].float()
            importance = img_hidden.norm(dim=-1)

            # 分离 keep / prune
            _, keep_local = importance.topk(n_keep)
            _, prune_local = importance.topk(n_prune, largest=False)

            keep_positions = img_positions[keep_local]
            prune_positions = img_positions[prune_local]

            keep_hidden = hidden_states[b, keep_positions, :]  # [n_keep, D]

            # 将每个被剪 token 替换为与其最相似的保留 token
            prune_hidden = hidden_states[b, prune_positions, :].float()
            keep_hidden_f = keep_hidden.float()
            
            # cosine similarity: [n_prune, n_keep]
            sim = torch.matmul(
                F.normalize(prune_hidden, dim=-1),
                F.normalize(keep_hidden_f, dim=-1).T
            )
            nearest = sim.argmax(dim=1)  # [n_prune]

            # scatter-copy: 被剪 token ← 最近的保留 token
            for i, pos in enumerate(prune_positions):
                hidden_states[b, pos, :] = keep_hidden[nearest[i]]
...
def load_qwen25vl(model_id, fastv_k=2, fastv_r=0.5):
    ...
    fastv = FastVWrapper(model, fastv_k=fastv_k, fastv_r=fastv_r)
    fastv.patch_generate()
    print(f"  FastV installed: K={fastv_k}, R={fastv_r}")

    return model, processor, fastv
        ...
        try:
            ...
            for i, item in enumerate(ds):
                ...
                if isinstance(choices, str):
                    ...
                    except json.JSONDecodeError: choices = [c.strip() for c in choices.split(",")]
                all_items.append({
...
def main():
    ...
    model, processor, fastv = load_qwen25vl(
        args.model_id, fastv_k=args.fastv_k, fastv_r=args.fastv_r
    )
    ...
    print(f"\n{'='*60}")
    print(f"  FastV (K={args.fastv_k}, R={args.fastv_r}) on Qwen2.5-VL — Summary")
    print(f"{'='*60}")
