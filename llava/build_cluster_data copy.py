"""
构建含有 focus_clusters 的训练数据 — LLaVA-OneVision 版。
只用问题文本让 Claude 选择需要关注的 cluster，不传图片节省费用。

用法：
    python llava/build_cluster_data_llava.py --layer 8 --max 5000
    python llava/build_cluster_data_llava.py --layer 8
"""
import os
import sys
...
def select_clusters_with_claude(
    question, candidate_clusters, client, retries=3,
) -> list:
    candidate_str = "\n".join([
        f"  {cid}: {name}"
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="data/llava_train_cluster.jsonl")
