...
用法：
    CUDA_VISIBLE_DEVICES=1 python src/v_new.py \
        --image data/images/train2014/COCO_train2014_000000524709.jpg \
        --question "What is looking out of the window of the car?" \
...
class HeatmapVisualizer:
    ...
    def _save_single_panel(self, img_data, label, save_path, tag):
        """保存单张子图为独立的 png + pdf"""
        H_px, W_px = img_data.shape[:2]
        panel_w = 3.2
        panel_h = panel_w * H_px / W_px
        fig, ax = plt.subplots(1, 1, figsize=(panel_w, panel_h + 0.5))
        plt.subplots_adjust(left=0.01, right=0.99, top=0.95, bottom=0.08)

        ax.imshow(img_data)
        #ax.set_xlabel(label, fontsize=16, labelpad=4)
    ...
    def plot(self, original_image, n_clusters=3, threshold=0.3, alpha=0.45,
        ...
        # ── 准备各 panel 数据 ──
        panels = []  # (img_data, label, tag)

        # 原图
        panels.append((img, "Input Image", "input"))
        ...
        panels.append((overlay, "Overall Focus", "overall"))
        ...
        for i, (cid, acts) in enumerate(sorted_clusters[:n_show]):
            ...
            panels.append((overlay_c, f"Neuron Cluster {cid}", f"cluster{cid}"))
        ...
        for idx, (img_data, label, _) in enumerate(panels):
            axes[idx].imshow(img_data)
            axes[idx].set_xlabel(label, fontsize=16, labelpad=4)
        ...
        if save_path:
            ...
            # 保存每张子图为独立文件
            for img_data, label, tag in panels:
                self._save_single_panel(img_data, label, save_path, tag)
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="outputs/heatmaps/abstract")
